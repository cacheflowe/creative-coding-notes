...
class Cell {
  ...
  void display() {
    ...
    color cellColor = color(.5+.5*sin(r) * cos(ctrlPt1/100) * cos(ctrlPt2/100) * sin(ctrlPt3/100), .2+.5*sin(g) * sin(ctrlPt1/100) * sin(ctrlPt2/100) * sin(ctrlPt3/100), .2+.5*cos(b) * sin(ctrlPt1/100) * cos(ctrlPt2/100) * sin(ctrlPt3/100) );
    // Color calculated using sine wave
    fill(cellColor);
    // adjust cell size per brightness
    float brightAdjust = brightness( cellColor ) * 8;
    rect(x,y,w,h); 
  }
}
